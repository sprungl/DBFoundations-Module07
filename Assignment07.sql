...
-- 2017-01-01,LSprung,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_LSprung')
	 Begin 
	  Alter Database [Assignment07DB_LSprung] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_LSprung;
	 End
	Create Database Assignment07DB_LSprung;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_LSprung;
...
Select 
	ProductName, 
	Format(InventoryDate, 'MMMM, yyyy') As InventoryDateFormatted, 
	Count
From vProducts
Inner Join vInventories
On vProducts.ProductID = vInventories.ProductID
...
--	Format(InventoryDate, 'MMMM, yyyy') As InventoryDateFormatted, 
--	Count, 
--	IIF(Month(InventoryDate) = 1, 0,14) As PreviousMonthCount, --January
--	Lag(Count, 1) Over (Order By ProductName, InventoryDate) As PreviousMonthCount --Lag
--From vProducts
--Inner Join vInventories
--On vProducts.ProductID = vInventories.ProductID;
...
Select 
	...
	Format(InventoryDate, 'MMMM, yyyy') As InventoryDateFormatted, 
	Count, 
	IIF(Month(InventoryDate) = 1, 0,Lag(Count, 1) Over (Order By ProductName, InventoryDate)) As PreviousMonthCount
From vProducts
Inner Join vInventories
On vProducts.ProductID = vInventories.ProductID;
go
